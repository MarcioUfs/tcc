...
import useStyles from './makeDash';
// import listCol from '../../functions/listColumns'
// import MyMenu from '../menu/myMenu';
import TableContainer from '@mui/material/TableContainer';
import Table from '@mui/material/Table';
import discharge from '../../../functions/listDischarge';
import api from '../../../services/api';
import MyFooter from '../menu/myFooter';
import './style.css'
...
function PessoasTable() {
    ...
    const [columns, setColumns] = useState({ coluna: listarCol() }); //dcod_familiar_fam
    ...
    const [limit, setLimit] = useState({ limits: 100 });
    ...
    function handleSubmit(event) {
        ...
        api.post('/search', values)
            .then(function (dat) {
                if (dat.status === 200) {
                    setResponse(dat.data.rows)
                }
            })
            .catch(function (err) {
                console.log(err);
        ...
        api.post('/dashboard/contar', values)
            .then(function (dat) {
                if (dat.status === 200) {
                    setNumberRows(dat.data.rows[0].count)
                }
            })
            .catch(function (err) {
                console.log(err);
    ...
    //     showValues()
    //     if (response[0]) {
    //         const valium = Object.keys(response[0])
    //         // discharge.map(nome =>(console.log((valium))))
    //         // console.log(typeof(valium))
    //         // console.log(apenasList)
    //     }
    // }, [response])
    return (
                    ...
                    <form onSubmit={handleSubmit} method="post">
                        ...
                        <select name="busca" onChange={(e) => handleChange(e)}>
                            {cabeca.map(function (nome, id) {
                                return (<option key={id} value={nome.codigo} >{nome.descricao}</option>)
                            })}
                        </select>
                        <h4>valor:</h4>
                        <input className="input" name="busca1" type="text" onChange={(e) => handleChange2(e)} placeholder="Pesquisar"></input>
                        <button type="submit" disabled={botao}>Enviar</button>
                            ...
                            <thead>
                                <tr>
                                    {showColumns().map((nome, index) => (
                                        <th key={index} value={index}>{nome}</th>
                                    ))}
                                </tr>
                            </thead>
