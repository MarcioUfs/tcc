...
var fs = require('fs');
function reader(arch_path, limit, execute) {
    var cont = 0;
    var fila = 0;
    function temVaga() {
        return fila < limit;
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        saida = line.map(function (valor, index) {
                            var entra = valor.replace(/['\,()/*]/gm, '').trim();
                            if (entra !== '') {
                                if (!isNaN(entra)) {
                                    if (index === 1 || index === 19 || index === 75 || index === 79 || index === 103 || index === 104 || index === 113 || index === 114 || index === 115) {
                                    }
                                    else {
                                        var verificador = parseInt(entra);
                        ...
                        fila++;
                        return [4 /*yield*/, execute(linha, cont++)];
                    case 1:
                        _a.sent();
                        fila--;
                        if (temVaga()) {
